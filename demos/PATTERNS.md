...
**Codex era (2026-08-25):** +48 explainers (18 + 30-extension) from the AppliedInteractive UX/UI Codex and AI-Era Programming Codex — prefixes `ux-` (9, chip UX Lab: Fitts/Hick/Doherty rigs that MEASURE the user with a ghost-hand idle mode; preattentive/change-blindness/gestalt; ot-crdt/presence/modes) and `sys-` (9, chip Systems: retry-storm/herd/breaker feedback loops; split-brain/consistency/merkle agreement; tail-latency/backpressure/shuffle-shard). Codex source distilled to scratchpad codex-sections/*.txt (66 files) — read those, never the 600KB pages. EXTENSION LESSON — the economics that worked: builders SELF-REVIEW their own screenshots against the design minimums before reporting (a builder reading 3 shots ≈ 5k tokens; the orchestrator fix-round it replaces ≈ 130k) — 9 consecutive trios then passed on one confirmation glance; run all trios in parallel, not waves; shared BRIEF-RULES.md in scratchpad instead of per-brief boilerplate; orchestrator batches wiring/push/evidence. Builders' self-verification caught real model bugs (quantized latencies, sign/pacing errors, an overclaimed p2c bound) — put the falsifiable law checks in every brief. New standing brief rules born this wave: stage headers/chips NEVER behind the overlay and always on separate rows (chip-overprint was the #1 QA failure, 6 of 9 fix rounds); experiment rigs must persist trial HISTORY on the stage; a queue/growth mechanism should GROW into empty stage space (the emptiness wastes the demo's own argument); dynamic chip placement must clamp away from header/banner bands.
...
## 8 · ML FIELD NOTES (2026-09 — the 23 ML/AI demos rebuilt as one curriculum)

- **One shell for the whole category**: `templates/ml-field-notes-shell.html` — top bar (← Gallery · ML/AI chip · title · keys), stage canvas (DPR-scaled via ResizeObserver, `touch-action:none`), stage caption + play/step/reset/speed dock, and a lesson panel: 3 stat tiles → tabs **Learn** (intuition + 5–7 step guided tour `STEPS[{t,d,cap,go()}]` + In the wild) · **Try this** (`TRIES[{q,run(),why}]` predict→run→why cards) · **The math** (`.eq` colour-coded equations with `.live` values) · **Controls** (`slider()`/`seg()` helpers). Keyboard: Space, ←/→ tour, R. dt clamped to [0,0.1]. `window.__demo={goStep,setPlaying,state}` for screenshot QA. Phone (<880px): stage 58vh on top, panel scrolls below.
- **Colour means the same thing in every ML demo** (transfer between lessons): coral `#ff7a6b` = class A/positive · teal `#3fd0c0` = class B/negative · amber `#ffc857` = the model's current parameters/guess/selection · violet `#a78bfa` = gradients/updates/errors · per-demo `--accent` for chrome only.
- **Layout law for the shell**: scene inside the safe rect `{x:16, y:84, w:W-32, h:H-160}` (caption above, dock below); labels placed by trying several candidate spots and scoring collisions (the recurring QA fix).
- **Process that worked**: shared brief + shell → one builder per demo (rebuild, node test of the core law between `/*CORE-START*/…/*CORE-END*/`, desktop/tour/phone self-screenshots) → one adversarial QA reviewer per demo that re-derives the maths, clicks every tour step and experiment, and fixes in place. Run as several small parallel workflows (a 4-CPU box caps a single workflow at 2 concurrent agents).
- **What QA kept catching** (put these in future briefs): tour steps inheriting stale state from a previous step/experiment (each `go()`/`run()` must set up its own state); captions/"why" text claiming something the simulation doesn't actually show (verify claims numerically — GELU "recovers", ACO "low ρ = premature convergence" were both false); experiments leaving the previous caption up; `.eq` lines overflowing the 380px panel; label overprint on phone; "You play" modes leaking hidden answers through live tiles.
